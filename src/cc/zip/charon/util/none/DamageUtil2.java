...
public class DamageUtil2 implements Util {
   public static boolean isArmorLow(EntityPlayer player, int durability) {
      Iterator var2 = player.inventory.armorInventory.iterator();
   ...
   public static boolean canBreakWeakness(EntityPlayer player) {
      int strengthAmp = 0;
      PotionEffect effect = Util.mc.player.getActivePotionEffect(MobEffects.STRENGTH);
      if (effect != null) {
         strengthAmp = effect.getAmplifier();
      }

      return !Util.mc.player.isPotionActive(MobEffects.WEAKNESS) || strengthAmp >= 1 || Util.mc.player.getHeldItemMainhand().getItem() instanceof ItemSword || Util.mc.player.getHeldItemMainhand().getItem() instanceof ItemPickaxe || Util.mc.player.getHeldItemMainhand().getItem() instanceof ItemAxe || Util.mc.player.getHeldItemMainhand().getItem() instanceof ItemSpade;
   }
   ...
   public static float calculateDamage(double posX, double posY, double posZ, Entity entity) {
      ...
      if (entity instanceof EntityLivingBase) {
         finald = (double)getBlastReduction((EntityLivingBase)entity, getDamageMultiplied(damage), new Explosion(Util.mc.world, (Entity)null, posX, posY, posZ, 6.0F, false, true));
      }
   ...
   public static boolean canTakeDamage(boolean suicide) {
      return !Util.mc.player.capabilities.isCreativeMode && !suicide;
   }
   ...
   public static int getCooldownByWeapon(EntityPlayer player) {
      ...
      if (item instanceof ItemSword) {
         ...
         return !(item instanceof ItemSpade) && item != Items.GOLDEN_AXE && item != Items.DIAMOND_AXE && item != Items.WOODEN_HOE && item != Items.GOLDEN_HOE ? 250 : 1000;
      } else {
         return 1250;
      }
   }
