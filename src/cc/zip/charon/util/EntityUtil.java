...
public class EntityUtil implements Util {
   ...
   public static boolean isObbyHole(BlockPos blockPos) {
      BlockPos[] var2 = new BlockPos[]{blockPos.north(), blockPos.south(), blockPos.east(), blockPos.west(), blockPos.down()};
   ...
   public static boolean isntValid(Entity entity, double range) {
      return entity == null || isDead(entity) || entity.equals(mc.player) || entity instanceof EntityPlayer && Charon.friendManager.isFriend(entity.getName()) || mc.player.getDistanceSq(entity) > MathUtil.square(range);
   }
   ...
   public static double getMaxSpeed() {
      ...
      if (mc.player.isPotionActive((Potion)Objects.requireNonNull(Potion.getPotionById(1)))) {
         maxModifier *= 1.0D + 0.2D * (double)(((PotionEffect)Objects.requireNonNull(mc.player.getActivePotionEffect((Potion)Objects.requireNonNull(Potion.getPotionById(1))))).getAmplifier() + 1);
      }
   ...
   public static Color getColor(Entity entity, int red, int green, int blue, int alpha, boolean colorFriends) {
      ...
      if (entity instanceof EntityPlayer && colorFriends && Charon.friendManager.isFriend((EntityPlayer)entity)) {
   ...
   public static boolean isMoving() {
      return (double)mc.player.moveForward != 0.0D || (double)mc.player.moveStrafing != 0.0D;
   }
   ...
   public static boolean checkCollide() {
      if (mc.player.isSneaking()) {
         return false;
      } else if (mc.player.getRidingEntity() != null && mc.player.getRidingEntity().fallDistance >= 3.0F) {
         return false;
      } else {
   ...
   public static Map<String, Integer> getTextRadarPlayers() {
      ...
      while(var5.hasNext()) {
         ...
         if (!player.isInvisible() && !player.getName().equals(mc.player.getName())) {
            ...
            ((Map)output).put(healthSB.toString() + " " + (Charon.friendManager.isFriend(player) ? ChatFormatting.AQUA : ChatFormatting.RED) + player.getName() + " " + distanceSB.toString() + " Â§f0", (int)mc.player.getDistance(player));
