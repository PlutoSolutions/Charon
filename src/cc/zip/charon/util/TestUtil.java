...
public class TestUtil {
   ...
   public static boolean canSeeBlock(BlockPos p_Pos) {
      return mc.player != null && mc.world.rayTraceBlocks(new Vec3d(mc.player.posX, mc.player.posY + (double)mc.player.getEyeHeight(), mc.player.posZ), new Vec3d((double)p_Pos.getX(), (double)p_Pos.getY(), (double)p_Pos.getZ()), false, true, false) == null;
   }
      ...
      if (isBlockEmpty(pos)) {
         ...
         for(int var4 = 0; var4 < var3; ++var4) {
            ...
            if (!emptyBlocks.contains(neighborBlock) && mc.player.getPositionEyes(mc.getRenderPartialTicks()).distanceTo(vec) <= 4.25D) {
               float[] var10000 = new float[]{mc.player.rotationYaw, mc.player.rotationPitch};
