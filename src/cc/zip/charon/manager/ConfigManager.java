...
import cc.zip.charon.features.setting.EnumConverter;
...
public class ConfigManager implements Util {
   ...
   public static void setValueFromJson(Feature feature, Setting setting, JsonElement element) {
      ...
      case 6:
         ...
         return;
      default:
         Charon.LOGGER.error("Unknown Setting type for: " + feature.getName() + " : " + setting.getName());
      }
   }
      ...
      while(true) {
         while(var2.hasNext()) {
            ...
            if (feature instanceof FriendManager) {
               try {
                  Charon.friendManager.addFriend(new FriendManager.Friend(element.getAsString(), UUID.fromString(settingName)));
   ...
   public void loadConfig(String name) {
      List<File> files = (List)Arrays.stream(Objects.requireNonNull((new File("charon")).listFiles())).filter(File::isDirectory).collect(Collectors.toList());
      if (files.contains(new File("charon/" + name + "/"))) {
         this.config = "charon/" + name + "/";
      } else {
         this.config = "charon/config/";
      }
   ...
   public boolean configExists(String name) {
      List<File> files = (List)Arrays.stream(Objects.requireNonNull((new File("charon")).listFiles())).filter(File::isDirectory).collect(Collectors.toList());
      return files.contains(new File("charon/" + name + "/"));
   }
   ...
   public void saveConfig(String name) {
      ...
      Charon.friendManager.saveFriends();
      Iterator var3 = this.features.iterator();
   ...
   public void init() {
      this.features.addAll(Charon.moduleManager.modules);
      this.features.add(Charon.friendManager);
      String name = this.loadCurrentConfig();
      this.loadConfig(name);
      Charon.LOGGER.info("Config loaded.");
   }
   ...
   private void loadPath(Path path, Feature feature) throws IOException {
      ...
      try {
         ...
         Charon.LOGGER.error("Bad Config File for: " + feature.getName() + ". Resetting...");
