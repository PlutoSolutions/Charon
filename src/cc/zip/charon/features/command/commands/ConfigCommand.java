...
public class ConfigCommand extends Command {
   ...
   public void execute(String[] commands) {
      if (commands.length == 1) {
         sendMessage("You`ll find the config files in your gameProfile directory under charon/config");
      } else {
         String configs;
         if (commands.length == 2) {
            if ("list".equals(commands[0])) {
               configs = "Configs: ";
               File file = new File("charon/");
         ...
         if (commands.length >= 3) {
            ...
            switch(var8) {
            case 0:
               Charon.configManager.saveConfig(commands[1]);
               sendMessage(ChatFormatting.GREEN + "Config '" + commands[1] + "' has been saved.");
               return;
            case 1:
               if (Charon.configManager.configExists(commands[1])) {
                  Charon.configManager.loadConfig(commands[1]);
